    ...
    try {
      ...
      transporter.sendMail(emailTemplate, (err, info) => {
        if (err) {
          ...
          return res.json({
            status: "error",
            message: "Error while sending verification link to user",
          });
        }
      ...
      try {
        ...
        return res.json({
          status: "error",
          message: "Error deleting invalid user or reference",
        });
      }
      return res.json({
        status: "error",
        message:
          "Reference could not be made properly or mailing the link failed",
      });
    }
  } catch (error) {
    if (error.code === 11000) {
      ...
      return res.json({
        status: "error",
        message: "Email Id for this event is already in use",
      });
    }
    throw error;
  }
...
router.get("/verify-user", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      status: "error",
      message: "Error fetching reference, Please try again",
    });
  }
  if (reference && reference.hash === hash) {
    try {
      ...
      return res.json({
        status: "error",
        message: "Invalid link, No user found or user already verified",
      });
    }
  } else {
...
router.post("/change-password", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      status: "error",
      message: "Error has occured while updating password",
    });
  }
});
  ...
  User.findOne({ email, event_id }, async (err, user) => {
    if (err) {
      ...
      if (user != null) {
        ...
        const token = await generateTokenFromPasswordHash(
          user._id,
          user.password,
          createdAt
        );
        ...
        transporter.sendMail(emailTemplate, (err, info) => {
          if (err) {
            ...
            return res.json({
              status: "error",
              message: "Error while sending link to user",
            });
          }
        ...
        return res.json({
          status: "error",
          message: "User with email not found for this Event",
        });
      }
    }
  });
